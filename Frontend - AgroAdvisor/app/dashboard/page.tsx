...
export default function DashboardPage() {
  ...
  async function fetchWeather() {
    try {
      ...
      if (wRes.status === "fulfilled")
        setWeather(wRes.value.data?.cuaca || wRes.value.data);
  ...
  async function fetchSaran() {
    ...
    try {
      ...
      if (alur2) {
        addAlert({
          level: "info",
          pesan: "Rekomendasi tanaman berhasil dianalisis.",
        });
      }
    } catch (_) {
  ...
  async function handleReset() {
    if (
      !window.confirm(
        "Reset sesi? Data lama tetap tersimpan untuk pembelajaran AI.",
      )
    )
      return;
    setResetting(true);
    try {
      ...
      addAlert({
        level: "info",
        pesan: "Sesi berhasil direset. Sesi baru dimulai.",
      });
      fetchSaran();
    } finally {
      setResetting(false);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-white/40 mt-1">
            {s.lokasi || "Lahan"} · Device:{" "}
            {s.device_id || "Menunggu sensor..."}
            {s.timestamp && (
      ...
      <div className="grid gap-4 grid-cols-2 md:grid-cols-3 lg:grid-cols-6">
        {[
          ...
          },
          {
            ...
            value: s.nitrogen,
            unit: "mg/kg",
            status: s.status_nitrogen,
          },
          {
            label: "Fosfor",
            value: s.fosfor,
            unit: "mg/kg",
            status: s.status_fosfor,
          },
          {
            label: "Kalium",
            value: s.kalium,
            unit: "mg/kg",
            status: s.status_kalium,
          },
          {
            label: "Suhu",
            value: s.suhu_udara,
            unit: "°C",
            status: s.suhu_udara > 35 ? "tinggi" : "normal",
          },
          {
            label: "Kelembaban",
            value: s.kelembaban_tanah,
            unit: "%",
            status: s.kelembaban_tanah < 30 ? "rendah" : "normal",
          },
        ].map((stat, idx) => {
          ...
          return (
            <Card
              key={idx}
              className="border-white/[0.06] bg-[#141b16] shadow-none backdrop-blur-sm"
            >
                  ...
                  {stat.value && (
                    <div
                      className={`h-1.5 w-1.5 rounded-full ${
                        stat.status === "optimal" || stat.status === "normal"
                          ? "bg-primary"
                          : stat.status === "rendah"
                            ? "bg-amber-500"
                            : "bg-red-500"
      ...
      <div className="grid gap-6 lg:grid-cols-2">
            ...
            <CardHeader className="pb-2 flex flex-row items-center justify-between">
              <div>
                ...
                <p className="text-xs text-white/40 mt-1">
                  {weather.lokasi || weather.kota || "Unknown"}
                </p>
              </div>
              <Button
                size="sm"
                ...
                <>
                  <div className="flex flex-col sm:flex-row items-center gap-6">
                    <div className="flex items-center gap-4">
                      {weather.ikon ? (
                        <img
                          src={`https://openweathermap.org/img/wn/${weather.ikon}@2x.png`}
                          alt={weather.deskripsi || "cuaca"}
                          className="w-16 h-16"
                        />
                      ) : (
                    ...
                    <div className="grid grid-cols-2 gap-4 flex-1 w-full sm:w-auto mt-4 sm:mt-0 pt-4 sm:pt-0 border-t sm:border-t-0 sm:border-l border-white/[0.06] sm:pl-6">
                      ...
                      <div>
                        ...
                        <div className="text-lg font-mono text-white/80">
                          {weather.kecepatan_angin || weather.angin} km/j
                        </div>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="mt-4 p-3 bg-blue-500/10 border border-blue-500/20 rounded-xl flex items-start gap-3 animate-in fade-in">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-[10px] text-blue-300/60 mt-0.5">
                          Sistem irigasi otomatis menunda jadwal penyiraman.
                        </p>
                      </div>
                    </div>
                  )}
        ...
        <Card className="border-white/[0.06] bg-white/[0.03] shadow-none backdrop-blur-sm lg:col-span-2">
          <CardHeader className="pb-2">
            <CardTitle className="text-base font-semibold text-white">
              Visualisasi Nutrisi Tanah (NPK)
            </CardTitle>
            <p className="text-xs text-white/40 mt-1">
              Estimasi ketersediaan Nitrogen, Fosfor, dan Kalium saat ini
              (mg/kg)
            </p>
          </CardHeader>
              ...
              <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
                <div className="p-4 rounded-lg border border-white/[0.06] bg-black/20 mb-4">
                  ...
                  <p className="text-sm text-white/80 leading-relaxed">
                    {saranTanaman.kondisi_ringkasan ||
                      "Kondisi tanah sesuai pembacaan sensor."}
                  </p>
                </div>
                ...
                {(() => {
                  ...
                  const leftItems = isExpanded
                    ? leftItemsFull
                    : leftItemsFull.slice(0, maxVisiblePerColumn);
                  const rightItems = isExpanded
                    ? rightItemsFull
                    : rightItemsFull.slice(0, maxVisiblePerColumn);
                  const showMoreButton =
                    leftItemsFull.length > maxVisiblePerColumn ||
                    rightItemsFull.length > maxVisiblePerColumn;

                  const renderItem = (rek: any, globalIdx: number) => (
                    <div
                      key={globalIdx}
                      className={`rounded-xl border border-white/[0.06] p-5 shadow-none backdrop-blur-md ${globalIdx === 0 ? "bg-primary/[0.03] border-primary/30" : "bg-white/[0.02]"}`}
                    >
                      <div className="flex items-start gap-3 mb-3">
                        ...
                        <div>
                          <h4 className="text-lg font-bold text-white tracking-wide capitalize">
                            {rek.jenis_tanaman}
                          </h4>
                      ...
                      <p className="text-sm text-white/70 leading-relaxed bg-black/20 p-3 rounded-lg border border-white/[0.03]">
                        {rek.alasan_cocok}
                      </p>
                    </div>
                  );
                  ...
                  return (
                    <div className="space-y-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-4">
                          {leftItems.map((rek: any, idx: number) =>
                            renderItem(rek, idx),
                          )}
                        </div>
                        <div className="space-y-4">
                          {rightItems.map((rek: any, idx: number) =>
                            renderItem(rek, half + idx),
                          )}
                        </div>
                      </div>
                        ...
                        <div className="flex justify-center pt-2">
                          ...
                          >
                            {isExpanded ? (
                              ...
                              <>
                                Tampilkan Semua ({items.length} Tanaman){" "}
        ...
        <Card className="border-white/[0.06] bg-white/[0.03] shadow-none backdrop-blur-sm">
          ...
          <CardContent>
            <div className="grid gap-3 sm:grid-cols-2">
              {[
                {
                  title: "Analisis Pupuk",
                  description: "Dapatkan rekomendasi pupuk AI",
                  href: "/dashboard/pupuk",
                },
                {
                  title: "Deteksi Pestisida",
                  description: "Upload gambar hama",
